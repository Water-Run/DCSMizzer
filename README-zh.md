...
> [!IMPORTANT]
>
> **当前状态（2026-07-31）：测绘/基础建设阶段。** `Tools`提供MIZ/CMP
> 检查；当前安装静态和真实任务观测证据查询；锁定上游缓存的准备和就绪检查；
> 认可且绑定提交、覆盖多地图的地形、机场、停机位、生成点、单位及挂点查询；
> 当前选项和仓库模板；常用触发器、
> 目标和定时文本的有限编译；原生MiG-29A GCI证据；构建规格证据审计；
> 确定性低层MIZ组装；完整场景严格结构与契约检查；以及回读验证。
> 自然语言场景规划、战役生成、完整运行时数据库导出、任务编辑器重存和DCS
> 运行验证均未实现。使用前先读[`Docs/index.txt`](./Docs/index.txt)，并运行
