...
| Content-addressed construction trace | `construction-snapshot` binds exact spec/resource bytes, audit/build/verify reports, MIZ bytes, embedded evidence/readiness preimages, producer/toolchain, and a recomputed three-stage hash DAG; `construction-verify` rechecks strict membership/hashes/continuity and byte-replays build/verification only under the exact producer/toolchain. Audit subquery decisions and DCS runtime are not replayed, and specs containing a native GCI station are conservatively refused until its install/manual/training inputs have a sealed evidence domain, so full reproducibility and release readiness remain false |
